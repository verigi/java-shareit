...
public class BookingServiceImpl extends CommonChecker implements BookingService {
    ...
    @Override
    @Transactional
    public BookingDto saveBooking(Long bookerId, BookingCreateDto bookingDto) {
        log.debug("Save booking request received. Booker id: {}", bookerId);
        Item item = checkItemAndReturn(bookingDto.getItemId());
        User user = checkUserAndReturn(bookerId);
    ...
    @Override
    @Transactional
    public BookingDto updateBooking(Long bookerId, BookingUpdateDto bookingUpdateDto) {
        ...
        boolean isBooker = updBooking.getBooker().getId().equals(bookerId);
    ...
    @Override
    @Transactional
    public BookingDto deleteBooking(Long bookerId, Long bookingId) {
    ...
    @Override
    @Transactional(readOnly = true)
    public Collection<BookingDto> findAllBookingsByOwnerAndState(Long ownerId, String state) {
        log.debug("Get all bookings of owner request received. Owner id: {}. State: {}", ownerId, state);
        ...
        switch (currState) {
            ...
            case CURRENT -> bookings = bookingRepository.findAllCurrentBookingByOwnerId(ownerId);
            case PAST -> bookings = bookingRepository.findAllPastBookingByOwnerId(ownerId);
            case FUTURE -> bookings = bookingRepository.findAllFutureBookingByOwnerId(ownerId);
            case WAITING -> bookings = bookingRepository.findAllByOwnerIdAndStatus(ownerId, Status.WAITING);
            case REJECTED -> bookings = bookingRepository.findAllByOwnerIdAndStatus(ownerId, Status.REJECTED);
            default -> throw new NoSuchStateException("Incorrect state");
        }
